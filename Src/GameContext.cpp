...
void GameContext::Init() {
        ...
        VkImageCreateInfo colour_image_info{
        ...
        VmaAllocationCreateInfo colour_allocation_info{ .flags = VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT, .usage = VMA_MEMORY_USAGE_AUTO };
        res = vmaCreateImage(vulkan_allocator, &colour_image_info, &colour_allocation_info, &colour_image, &colour_allocation, nullptr);
        ...
        res = vkCreateImageView(vulkan_device, &colour_view_info, nullptr, &colour_image_view);
        ...
        std::vector<VkVertexInputAttributeDescription> vertex_attributes{
                ...
                { .location = 7, .binding = 1, .format = VK_FORMAT_R32G32B32A32_SFLOAT, .offset = offsetof(InstanceDrawData, colour0) },
                { .location = 8, .binding = 1, .format = VK_FORMAT_R32G32B32A32_SFLOAT, .offset = offsetof(InstanceDrawData, colour1) },
                { .location = 9, .binding = 1, .format = VK_FORMAT_R32_UINT, .offset = offsetof(InstanceDrawData, user_value) },
                { .location = 10, .binding = 1, .format = VK_FORMAT_R32_UINT, .offset = offsetof(InstanceDrawData, user_value1) },
                { .location = 11, .binding = 1, .format = VK_FORMAT_R32_UINT, .offset = offsetof(InstanceDrawData, user_value2) },
                { .location = 12, .binding = 1, .format = VK_FORMAT_R32_UINT, .offset = offsetof(InstanceDrawData, user_value3) },
        };
...
void GameContext::Resize() {
        ...
        vkDestroyImageView(vulkan_device, colour_image_view, nullptr);
        vmaDestroyImage(vulkan_allocator, colour_image, colour_allocation);
...
void GameContext::Render(const Camera& camera) {
        ...
        std::array<VkImageMemoryBarrier2, 3> output_barriers{
        ...
        // Acquire all needed buffers

        std::vector<VkSemaphore> wait_semaphores{
                present_semaphores[frame_index],
        };

        model_system.Acquire(command_buffer, wait_semaphores);
        ...
        std::vector<VkPipelineStageFlags> wait_stages = { VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT };

        for (u32 i = 0; i < wait_semaphores.size(); i++) {
                wait_stages.push_back(VK_ACCESS_2_VERTEX_ATTRIBUTE_READ_BIT);
        }
